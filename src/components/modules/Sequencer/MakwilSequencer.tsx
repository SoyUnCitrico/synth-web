...
interface MakwilSequencerProps {
  configs: SeqConfig[];
  ...
  cv4Steps: CvStep[];
  setCv4Steps: React.Dispatch<React.SetStateAction<CvStep[]>>;
  currentSteps: number[];
}

const NOTE_NAMES = ['C', 'C#', 'D', 'D#', 'E', 'F', 'F#', 'G', 'G#', 'A', 'A#', 'B'];
// Etiqueta de nota incluyendo el desplazamiento de octava del secuenciador.
const noteLabel = (offset: number, octave: number): string => {
  const semi = SEQ_ROOT_MIDI + offset + octave * 12;
...
// Controles por secuenciador: pasos, dirección, reloj y (sólo en los de pitch) botones de
// octava +/−. El reloj del seq 1 es la base; los demás tienen divisor/multiplicador.
const SeqControls: React.FC<{
  index: number;
  config: SeqConfig;
  hasOctave: boolean;
  onChange: (patch: Partial<SeqConfig>) => void;
}> = ({ index, config, hasOctave, onChange }) => (
  <div className="seq-config">
    ...
    {hasOctave && (
      <div className="control-group">
        <label>Octava: {config.octave}</label>
        <div className="seq-oct-btns">
          <button
            className="seq-btn seq-oct-btn"
            onClick={() => onChange({ octave: Math.max(0, config.octave - 1) })}
            disabled={config.octave <= 0}
          ...
          <button
            className="seq-btn seq-oct-btn"
            onClick={() => onChange({ octave: Math.min(MAX_OCTAVE, config.octave + 1) })}
            disabled={config.octave >= MAX_OCTAVE}
...
/**
 * Secuenciador de MAKWIL: 5 canales. seq1 = pitch (base); seq2 y seq3 = Nota + CV; seq4 y
 * seq5 = CV. Los de pitch (1,2,3) tienen botones de octava que desplazan la nota de sus pasos.
 */
const MakwilSequencer: React.FC<MakwilSequencerProps> = ({
  configs,
  setConfigs,
  bpm,
  setBpm,
  ...
  // Editores de los cuatro secuenciadores de CV (seq2..seq5). Los dos primeros tienen nota.
  const cvData = [
    { steps: cvSteps, setSteps: setCvSteps, hasPitch: true },
    { steps: cv2Steps, setSteps: setCv2Steps, hasPitch: true },
    { steps: cv3Steps, setSteps: setCv3Steps, hasPitch: false },
    { steps: cv4Steps, setSteps: setCv4Steps, hasPitch: false },
  ];
  ...
  return (
    <div className="module sequencer-module">
      ...
      <div className="module-controls">
        ...
        {/* Secuenciador 1: pitch (reloj base, con octava). */}
        <div className="seq-channel">
          <span className="seq-channel-title">Seq 1 · Pitch (base)</span>
          <SeqControls index={0} config={configs[0]} hasOctave onChange={(p) => updateConfig(0, p)} />
          <SliderLane
            label="Nota"
            ...
            step={1}
            tall
            valueLabel={(v) => noteLabel(v, configs[0].octave)}
        ...
        {/* Secuenciadores 2-5. seq2 y seq3 (which 0,1) añaden hilera de Nota (+ octava). */}
        {cvData.map((cv, which) => {
            ...
            <div className="seq-channel" key={seqIndex}>
              ...
              {hasPitch && (
                <SliderLane
                  ...
                  step={1}
                  tall
                  valueLabel={(v) => noteLabel(v, configs[seqIndex].octave)}
...
export default MakwilSequencer;
